...
from pocl import tem_forward_kl, forward_kl, tem_reverse_kl,reverse_kl, tem_js_distance, js_distance, tem_tv_distance, tv_distance
from pocl import tem_skewed_forward_kl,skewed_forward_kl, skewed_reverse_kl, tem_skewed_reverse_kl,pro_skewed_forward_kl_2,pro_skewed_forward_kl_1
from pocl import SampleGenerator, ReplayBuffer
...
def get_distil_loss(args, tokenizer, model, teacher_model, model_batch, no_model_batch, logits,temperature_kld):
    with torch.no_grad():
        teacher_model.eval()
    ...
    if args.model_parallel:
        ...
        if "tfkl" in args.type:
            distil_loss = tem_forward_kl(logits, teacher_logits, no_model_batch,temperature_kld)
        elif "trkl" in args.type:
            distil_loss = tem_reverse_kl(logits, teacher_logits, no_model_batch,temperature_kld)
        elif "tsfkl" in args.type:
            distil_loss = tem_skewed_forward_kl(logits, teacher_logits, no_model_batch,temperature_kld,lam=args.skew_alpha)
        elif "tsrkl" in args.type:
            distil_loss= tem_skewed_reverse_kl(logits, teacher_logits, no_model_batch, temperature_kld, lam=args.skew_alpha)
        elif "tjsd" in args.type:
            distil_loss = tem_js_distance(logits, teacher_logits, no_model_batch,temperature_kld)
        elif "ttvd" in args.type:
            distil_loss = tem_tv_distance(logits, teacher_logits, no_model_batc,temperature_kld)
...
def finetune(args, tokenizer: AutoTokenizer, model: deepspeed.DeepSpeedEngine, optimizer: AdamW, lr_scheduler, dataset, device, temperature_kld,teacher_model=None):
    ...
    for epoch in range(args.epochs):
        ...
        for it, (model_batch, no_model_batch, gen_data) in enumerate(train_dataloader):
            ...
            if "adaptive" in args.type:
                ...
                samp_threshold = None 
                # Explicitly set to None if not in adaptive mode
            ...
            if teacher_model is not None:
                distil_loss = get_distil_loss(args, tokenizer, model, teacher_model, model_batch, no_model_batch, logits,temperature_kld)
...
def main():
    ...
    if args.do_train:
        model = finetune(args, tokenizer, model, optimizer, lr_scheduler, dataset, device, temperature_kld=args.temperature_kld,teacher_model=teacher_model)
